...
router.delete('/user/:id', async (req, res) => {
  try {
    const deleted = await User.findByIdAndDelete(req.params.id);  // Finds and deletes by MongoDB _id
    if (!deleted)
    ...
    res.status(500).json({ message: 'Error deleting user', error }); // Server error
  }
});
